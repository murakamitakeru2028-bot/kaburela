    ...
    <div className="h-full min-h-0 px-2 py-5 sm:px-4 lg:px-6 flex flex-col gap-5">
      <header className="shrink-0 flex items-start justify-between gap-5 flex-wrap">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-[24px] font-semibold text-ink tracking-[-0.6px]">相関トレンド</h2>
      ...
      <div className="shrink-0 grid gap-2 lg:grid-cols-[minmax(220px,1fr)_auto_auto]">
        ...
        <div className="h-10 rounded-full bg-paper border border-border px-4 flex items-center gap-3">
          ...
          <input
            ...
            className="w-28 accent-[var(--color-ink)]"
          />
          <span className="text-[12px] font-mono text-ink tabular-nums w-9">{threshold.toFixed(2)}</span>
        </div>
      </div>

      <section className="min-h-0 flex-1 rounded-[18px] border border-border bg-paper overflow-hidden flex flex-col">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-4 shrink-0">
          ...
          <span className="text-[11px] text-muted font-mono tabular-nums">{visiblePairs.length} rows</span>
        </div>

        <div className="hidden lg:grid grid-cols-[56px_minmax(0,1fr)_100px_100px_160px_92px] gap-3 px-5 py-2.5 border-b border-border text-[10px] text-muted font-mono tracking-[0.08em] uppercase shrink-0">
          <span>Rank</span>
          <span>Pair</span>
...
function SegmentedSwitch<T extends string>({
  ...
  return (
    <div className="h-10 flex items-center bg-subtle rounded-full p-1 gap-1">
      {options.map(option => (
        <button
          key={option.id}
          type="button"
...
function TrendRow({
  ...
  return (
    <div className="grid grid-cols-[40px_minmax(0,1fr)_72px] lg:grid-cols-[56px_minmax(0,1fr)_100px_100px_160px_92px] gap-3 items-center px-4 lg:px-5 py-3 hover:bg-subtle/70 transition-colors">
...
function StockButton({
  ...
  return (
    <button
      ...
      className={cn('min-w-0 cursor-pointer group', align === 'right' ? 'text-right' : 'text-left')}
...
function CorrValue({ value, className }: { value: number; className?: string }) {
  return (
    <span className={cn('text-[13px] font-mono font-semibold tabular-nums', className)} style={{ color: value >= 0 ? 'var(--color-pos)' : 'var(--color-neg)' }}>
      {corrLabel(value)}
    </span>
  )
}
...
function DeltaBadge({ delta }: { delta: number }) {
  return (
    <span
      className="justify-self-end h-8 min-w-[70px] px-2 rounded-full flex items-center justify-center text-[13px] font-semibold font-mono tabular-nums"
      style={{
        color: deltaColor(delta),
        backgroundColor: delta >= 0 ? 'rgba(22, 163, 74, 0.1)' : 'rgba(220, 38, 38, 0.1)',
      }}
    >
      {deltaLabel(delta)}
    </span>
